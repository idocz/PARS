...
albedo_image = np.array(Image.open(join("data","technion.jpg")))[:,:,:3]
...
tensorboard = True
if tensorboard:
    title = datetime.now().strftime("%d%m-%H%M-%S") \
            + f"_Nr={resample_freq}_tosort={int(to_sort)}_ss={step_size}"
...
for iter in range(iteration):
    ...
    print(f"iter {iter}:  dist:{dist}, loss:{np.sum((I_res-I_gt)**2)} grad_norm:{np.linalg.norm(total_grad)}")
    ...
    if iter % plot_freq == 0:
        ...
        tb.update_albedo_image(I_res, loss, rel_dist1, albedo_image_res, iter)
