...
class SceneRecycling(object):
    def __init__(self, objects, camera):
        ...
        @cuda.jit()
        def generate_paths_cuda(object_mat, camera_location, width, height, spp, rng_states, depth_inds, hit_points,
                                hit_objects):
            i, j, s = cuda.grid(3)
            tid = height * width * s + width * j + i
            if i < width and j < height and s < spp:
        ...
        @cuda.jit()
        def render_cuda(Ks0, n0, hit_points, hit_objects, depth_inds, object_mat, camera_location, width, height, spp
                        , I_res):
            i, j, s = cuda.grid(3)
            tid = height * width * s + width * j + i
            if i < width and j < height and s < spp:
        ...
        @cuda.jit()
        def render_inverse_specular_cuda(Ks0, n0, hit_points, hit_objects, depth_inds, object_mat, camera_location, width,
                                      height, spp, rng_states, I_diff, total_grad):
            i, j, s = cuda.grid(3)
            tid = height * width * s + width * j + i
            if i < width and j < height and s < spp:
    ...
    def generate_paths(self, spp, init_rng=True, to_sort=False):
        ...
        self.calc_total_reflection[blockspergrid, threadsperblock](self.dobject_mat, self.dcamera_location, self.camera.width,
        ...
        self.dhit_objects = cuda.to_device(np.empty((2, total_depth), np.uint8))

        self.rng_states.copy_to_device(rng_states_cpu)
    ...
    def render(self, I_gt=None, inverse_type=None):
        ...
        blockspergrid = (blockspergrid_x, blockspergrid_y, blockspergrid_z)
        start = time()
        self.render_cuda[blockspergrid, threadsperblock](self.Ks0, self.n0, self.dhit_points, self.dhit_objects, self.ddepth_inds,
        ...
        if inverse_type == "color":
            ...
            exit(1)
        inverse_func(self.Ks0, self.n0, self.dhit_points, self.dhit_objects, self.ddepth_inds,
                                                         self.dobject_mat, self.dcamera_location, self.camera.width,
